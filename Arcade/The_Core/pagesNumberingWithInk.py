def solution(current, numberOfDigits):
    # Loop until there are not enough digits to print the current page number
    while numberOfDigits >= len(str(current)):
        numberOfDigits -= len(str(current)) # Deduct the digits required for the current page
        current += 1 # Move to the next page
    # Return the last numbered page (the page just before the one that cannot be numbered)
    return current - 1
